...
int P2PBase::get_socket(string ip, string port, bool is_server)
{
  ...
  if(ip != "")
    {
      vector<char> cip_vec(ip.begin(), ip.end());
      cip_vec.push_back('\0');
      cip = &*cip_vec.begin();
    }
...
bool P2PBase::add_host(int socket)
{
	...
	if(new_sock == -1)
	  {
	    printf("Error creating socket for new host connection");
	    return false;
	  }
	FD_SET(new_sock, &master);
	if(new_sock > fdmax)
	  fdmax = new_sock;
	inet_ntop(remote.ss_family,(sockaddr*)&remote, remote_ip, INET6_ADDRSTRLEN);
	
	char hostname[1024];
	char service[20];
	getnameinfo((sockaddr*)&remote, sizeof(remote), hostname, sizeof(hostname), service, sizeof(service), 0);
